...
class PeerDiscovery {
    ...
    async runFindRandomNodeQuery() {
        ...
        const randomNodeId = crypto_1.default.randomBytes(64).toString("hex");
    ...
    async handleDiscoveredPeer(enr) {
        try {
            ...
            const attnetsBytes = enr.get(metadata_1.ENRKey.attnets);
            const syncnetsBytes = enr.get(metadata_1.ENRKey.syncnets);
            // TODO: Optimize bitfield parsing
            const attnets = attnetsBytes ? lodestar_types_1.ssz.phase0.AttestationSubnets.deserialize(attnetsBytes) : [];
            const syncnets = syncnetsBytes ? lodestar_types_1.ssz.altair.SyncSubnets.deserialize(syncnetsBytes) : [];
            // Should dial peer?
            const cachedPeer = {
                peerId,
                multiaddrTCP,
