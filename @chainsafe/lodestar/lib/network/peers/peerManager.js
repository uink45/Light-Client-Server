...
class PeerManager {
    ...
    onStatus(peer, status) {
        ...
        try {
            (0, utils_1.assertPeerRelevance)(status, this.chain);
        }
        catch (e) {
            if (e instanceof utils_1.IrrelevantPeerError) {
                this.logger.debug("Irrelevant peer", { peer: (0, util_1.prettyPrintPeerId)(peer), reason: e.getMetadata() });
            }
            else {
                this.logger.error("Unexpected error in assertPeerRelevance", { peer: (0, util_1.prettyPrintPeerId)(peer) }, e);
            }
            if (peerData)
