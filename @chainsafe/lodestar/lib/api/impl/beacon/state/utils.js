...
exports.getStateValidatorIndex = exports.filterStateValidatorsByStatuses = exports.getSyncCommittees = exports.getEpochBeaconCommittees = exports.toValidatorResponse = exports.getValidatorStatus = exports.resolveStateId = void 0;
 ...
 */
function getEpochBeaconCommittees(state, epoch) {
    if (state.epochCtx !== undefined) {
        const stateEpoch = (0, lodestar_beacon_state_transition_1.computeEpochAtSlot)(state.slot);
        switch (epoch) {
            case stateEpoch:
                return state.currentShuffling.committees;
            case stateEpoch - 1:
                return state.previousShuffling.committees;
            // default: continue to manual computation below
        }
    }
    const activeValidatorIndices = [];
    for (const [i, v] of Array.from((0, ssz_1.readonlyValues)(state.validators)).entries()) {
        if ((0, lodestar_beacon_state_transition_1.isActiveValidator)(v, epoch)) {
            activeValidatorIndices.push(i);
        }
    }
    const shuffling = lodestar_beacon_state_transition_2.allForks.computeEpochShuffling(state, activeValidatorIndices, epoch);
    return shuffling.committees;
}
exports.getEpochBeaconCommittees = getEpochBeaconCommittees;
/**
 * Returns committees as an array of validator index
 */
function getSyncCommittees(state, epoch) {
    const statePeriod = (0, lodestar_beacon_state_transition_1.computeAbsoluteSyncPeriodAtSlot)((0, lodestar_beacon_state_transition_1.computeEpochAtSlot)(state.slot));
    const requestPeriod = (0, lodestar_beacon_state_transition_1.computeAbsoluteSyncPeriodAtSlot)(epoch);
    if (state.epochCtx !== undefined) {
        switch (requestPeriod) {
            case statePeriod:
                return state.currentSyncCommittee.validatorIndices;
            case statePeriod + 1:
                return state.nextSyncCommittee.validatorIndices;
            default:
                throw new errors_1.ApiError(400, "Epoch out of bounds");
        }
    }
    throw new errors_1.ApiError(400, "No CachedBeaconState available");
}
exports.getSyncCommittees = getSyncCommittees;
