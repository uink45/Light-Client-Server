...
class BeaconChain {
    constructor(opts, { config, db, logger, metrics, anchorState, eth1, executionEngine, }) {
        ...
        const forkChoice = (0, forkChoice_1.initializeForkChoice)(config, emitter, clock.currentSlot, cachedState, metrics);
    ...
    persistInvalidSszObject(type, bytes, suffix = "") {
        ...
        if (!fs_1.default.existsSync(byDate)) {
            fs_1.default.mkdirSync(byDate, { recursive: true });
        }
        const fileName = `${byDate}/${type}_${suffix}_${Date.now()}.ssz`;
        fs_1.default.writeFileSync(fileName, bytes);
        return fileName;
    }
}
