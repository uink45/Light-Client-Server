 ...
 */
function initializeForkChoice(config, emitter, currentSlot, state, metrics) {
    ...
    const justifiedBalances = (0, lodestar_beacon_state_transition_1.getEffectiveBalances)(state);
    return new lodestar_fork_choice_1.ForkChoice(config, new lodestar_fork_choice_1.ForkChoiceStore(currentSlot, justifiedCheckpoint, finalizedCheckpoint, {
        ...
        executionPayloadBlockHash: lodestar_beacon_state_transition_1.merge.isMergeStateType(state)
            ? (0, ssz_1.toHexString)(state.latestExecutionPayloadHeader.blockHash)
            : null,
