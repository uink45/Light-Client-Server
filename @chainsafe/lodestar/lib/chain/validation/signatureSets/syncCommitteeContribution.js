...
function getSyncCommitteeContributionSignatureSet(state, contribution, pubkeys) {
    ...
    return {
        type: lodestar_beacon_state_transition_2.SignatureSetType.aggregate,
        pubkeys,
        signingRoot: (0, lodestar_beacon_state_transition_2.computeSigningRoot)(lodestar_types_1.ssz.Root, contribution.beaconBlockRoot, domain),
...
function getContributionPubkeys(state, contribution) {
    ...
    const syncCommittee = lodestar_beacon_state_transition_1.allForks.getIndexedSyncCommittee(state, contribution.slot);
