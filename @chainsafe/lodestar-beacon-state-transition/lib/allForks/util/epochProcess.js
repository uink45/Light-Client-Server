...
const attesterStatus_1 = require("./attesterStatus");
...
function beforeProcessEpoch(state) {
    ...
    const baseRewardPerIncrement = (0, misc_1.computeBaseRewardPerIncrement)(totalActiveStakeByIncrement);
    // To optimize process_registry_updates():
    ...
    if (forkName === lodestar_params_1.ForkName.phase0) {
        (0, processPendingAttestations_1.statusProcessEpoch)(state, statuses, state.previousEpochAttestations, prevEpoch, attesterStatus_1.FLAG_PREV_SOURCE_ATTESTER, attesterStatus_1.FLAG_PREV_TARGET_ATTESTER, attesterStatus_1.FLAG_PREV_HEAD_ATTESTER);
        (0, processPendingAttestations_1.statusProcessEpoch)(state, statuses, state.currentEpochAttestations, currentEpoch, attesterStatus_1.FLAG_CURR_SOURCE_ATTESTER, attesterStatus_1.FLAG_CURR_TARGET_ATTESTER, attesterStatus_1.FLAG_CURR_HEAD_ATTESTER);
    }
    else {
        state.previousEpochParticipation.forEachStatus((participationStatus, i) => {
            ...
            if (isActivePrevEpoch[i]) {
                status.flags |=
                    (participationStatus.timelySource && attesterStatus_1.FLAG_PREV_SOURCE_ATTESTER) |
                        (participationStatus.timelyTarget && attesterStatus_1.FLAG_PREV_TARGET_ATTESTER) |
                        (participationStatus.timelyHead && attesterStatus_1.FLAG_PREV_HEAD_ATTESTER);
            }
        });
        state.currentEpochParticipation.forEachStatus((participationStatus, i) => {
            ...
            if (status.active) {
                status.flags |=
                    (participationStatus.timelySource && attesterStatus_1.FLAG_CURR_SOURCE_ATTESTER) |
                        (participationStatus.timelyTarget && attesterStatus_1.FLAG_CURR_TARGET_ATTESTER) |
                        (participationStatus.timelyHead && attesterStatus_1.FLAG_CURR_HEAD_ATTESTER);
            }
        });
    }
