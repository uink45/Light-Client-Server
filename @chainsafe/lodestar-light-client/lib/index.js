...
class Lightclient {
    constructor({ config, logger, genesisData, beaconApiUrl, snapshot }) {
        ...
        const periodCurr = (0, clock_2.computeSyncPeriodAtSlot)(this.config, snapshot.header.slot);
    ...
    async sync(fromPeriod, toPeriod) {
        ...
        for (const [fromPeriodRng, toPeriodRng] of periodRanges) {
            ...
            for (const update of updates) {
                ...
                const headPeriod = (0, clock_2.computeSyncPeriodAtSlot)(this.config, update.header.slot);
    ...
    async runLoop() {
        ...
        while (true) {
            const currentPeriod = (0, clock_2.computeSyncPeriodAtSlot)(this.config, this.currentSlot);
            ...
            if (!this.syncCommitteeByPeriod.has(currentPeriod)) {
                ...
                const headPeriod = (0, clock_2.computeSyncPeriodAtSlot)(this.config, this.head.header.slot);
            ...
            if (lodestar_params_1.EPOCHS_PER_SYNC_COMMITTEE_PERIOD - epochsIntoPeriod <= LOOKAHEAD_EPOCHS_COMMITTEE_SYNC) {
                const period = (0, clock_2.computeSyncPeriodAtEpoch)(this.config, currentEpoch);
                try {
    ...
    processHeaderUpdate(headerUpdate) {
        ...
        const period = (0, clock_2.computeSyncPeriodAtSlot)(this.config, header.slot);
    ...
    processSyncCommitteeUpdate(update) {
        ...
        const updatePeriod = (0, clock_2.computeSyncPeriodAtSlot)(this.config, updateSlot);
        ...
        const newNextSyncCommitteeStats = {
            isFinalized,
            participation: (0, utils_1.sumBits)(update.syncCommitteeBits),
            slot: updateSlot,
        };
