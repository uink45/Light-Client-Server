...
class SyncCommitteeDutiesService {
    ...
    async getDutiesAtSlot(slot) {
        const period = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtSlot)(this.config, slot + 1); // See note above for the +1 offset
        const duties = [];
    ...
    async pollSyncCommittees(currentEpoch, indexArr) {
        ...
        const currentPeriod = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtEpoch)(this.config, currentEpoch);
    ...
    async pollSyncCommitteesForEpoch(epoch, indexArr) {
        ...
        const period = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtEpoch)(this.config, epoch);
        let count = 0;
        ...
        this.logger.debug("Downloaded SyncDuties", { epoch, dependentRoot: (0, ssz_1.toHexString)(dependentRoot), count });
    }
