...
async function runNodeNotifier({ network, chain, sync, config, logger, signal, }) {
    ...
    try {
        while (!signal.aborted) {
            ...
            const isMergeComplete = lodestar_beacon_state_transition_1.merge.isMergeStateType(headState) && lodestar_beacon_state_transition_1.merge.isMergeComplete(headState);
            ...
            switch (sync.state) {
                ...
                case sync_1.SyncState.Synced: {
                    ...
                    nodeState = ["Synced", clockSlotRow, headRow, ...mergeInfo, finalizedCheckpointRow, peersRow];
                    break;
                }
                case sync_1.SyncState.Stalled: {
                    // Searching peers - peers - head - finalized - clock
                    nodeState = ["Searching peers", peersRow, clockSlotRow, headRow, ...mergeInfo, finalizedCheckpointRow];
                }
            }
