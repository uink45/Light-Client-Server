...
class BackfillSync extends events_1.EventEmitter {
    constructor(modules, opts) {
        ...
        this.opts = opts !== null && opts !== void 0 ? opts : { batchSize: BATCH_SIZE };
    ...
    async sync() {
        ...
        for await (const _ of this.processor) {
            ...
            try {
                ...
                this.logger.error("BackfillSync sync error", {}, e);
    ...
    async checkIfCheckpointSyncedAndValidate() {
        ...
        if (this.wsCheckpointHeader.slot >= this.syncAnchor.lastBackSyncedBlock.slot) {
            ...
            if (!wsDbCheckpointBlock ||
                Math.floor(wsDbCheckpointBlock.message.slot / lodestar_params_1.SLOTS_PER_EPOCH) !==
                    this.wsCheckpointHeader.slot / lodestar_params_1.SLOTS_PER_EPOCH)
    ...
    async checkUpdateFromBackfillSequences() {
        ...
        if (filteredSeqs.length > 0) {
            await this.db.backfilledRanges.batchDelete(filteredSeqs.filter((entry) => entry.key !== this.backfillStartFromSlot).map((entry) => entry.key));
        }
        return validSequence;
    }
    async fastBackfillDb() {
        ...
        while (backCount !== this.opts.batchSize &&
    ...
    async syncRange(peer) {
        ...
        const fromSlot = Math.max(toSlot - this.opts.batchSize, this.prevFinalizedCheckpointBlock.slot, constants_1.GENESIS_SLOT);
