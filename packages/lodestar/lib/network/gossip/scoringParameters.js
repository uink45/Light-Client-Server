...
function getAllTopicsScoreParams(config, eth2Context, precomputedParams) {
    ...
    const { currentFork, nextFork } = (0, forks_1.getCurrentAndNextFork)(config, epoch - forks_1.FORK_EPOCH_LOOKAHEAD - 1);
    const topicsParams = {};
...
function expectedAggregatorCountPerSlot(activeValidatorCount) {
    const committeesPerSlot = lodestar_beacon_state_transition_1.allForks.computeCommitteeCount(activeValidatorCount);
