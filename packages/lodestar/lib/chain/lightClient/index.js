...
class LightClientServer {
    constructor(modules, genesisData) {
        ...
        this.zero = {
            finalityHeader: lodestar_types_1.ssz.phase0.BeaconBlockHeader.defaultValue(),
    ...
    async getCurrentSyncCommitteeProof(state){
      const syncCommitteeWitness = await proofs_1.getSyncCommitteesWitness(state);
      const proof = await proofs_1.getCurrentSyncCommitteeBranch(syncCommitteeWitness);
      return proof;
    }
    ...
    async persistPostBlockImportData(block, postState, parentBlock) {
        ...
        const parentBlockPeriod = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtSlot)(this.config, parentBlock.slot);
        const period = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtSlot)(this.config, blockSlot);
        ...
        const finalizedCheckpointPeriod = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtEpoch)(this.config, finalizedCheckpoint.epoch);
    ...
    async onSyncAggregate(syncAggregate, signedBlockRoot) {
        ...
        const headerUpdate = { header: attestedData.header, syncAggregate };
        // Emit update
    ...
    async maybeStoreNewBestPartialUpdate(syncAggregate, attestedData) {
        const period = (0, lodestar_beacon_state_transition_1.computeSyncPeriodAtSlot)(this.config, attestedData.header.slot);
        ...
        const newPartialUpdate = attestedData.isFinalized
            ? {
                ...attestedData,
                finalizedHeader: await this.getFinalizedHeader(attestedData.finalizedCheckpoint.root),
    ...
    async getFinalizedHeader(finalizedBlockRoot) {
        ...
        if (!finalizedHeader) {
            throw Error(`finalityHeader not available ${(0, ssz_1.toHexString)(finalizedBlockRoot)}`);
        }
