...
async function importBlock(chain, fullyVerifiedBlock) {
    ...
    const onBlockPrecachedData = { executionStatus };
    if (justifiedCheckpoint.epoch > chain.forkChoice.getJustifiedCheckpoint().epoch) {
        ...
        onBlockPrecachedData.justifiedBalances = (0, lodestar_beacon_state_transition_1.getEffectiveBalances)(state);
    }
    if (lodestar_beacon_state_transition_1.merge.isMergeStateType(postState) &&
        lodestar_beacon_state_transition_1.merge.isMergeBlockBodyType(block.message.body) &&
        lodestar_beacon_state_transition_1.merge.isMergeBlock(postState, block.message.body)) {
