...
function onForkVersion(version) {
    this.logger.verbose("New fork version", lodestar_types_1.ssz.Version.toJson(version));
}
exports.onForkVersion = onForkVersion;
function onCheckpoint(cp, state) {
    var _a;
    this.logger.verbose("Checkpoint processed", lodestar_types_1.ssz.phase0.Checkpoint.toJson(cp));
...
function onJustified(cp, state) {
    var _a, _b;
    this.logger.verbose("Checkpoint justified", lodestar_types_1.ssz.phase0.Checkpoint.toJson(cp));
...
async function onFinalized(cp) {
    var _a;
    this.logger.verbose("Checkpoint finalized", lodestar_types_1.ssz.phase0.Checkpoint.toJson(cp));
...
async function onBlock(block, _postState) {
    ...
    this.logger.verbose("Block processed", {
        ...
        root: (0, ssz_1.toHexString)(blockRoot),
    });
}
...
async function onErrorBlock(err) {
    ...
    if (err.type.code === errors_1.BlockErrorCode.INVALID_SIGNATURE) {
        ...
        const preStatePath = this.persistInvalidSszObject("state", preState.serialize(), `${preState.slot}_invalid_state_root_preState_${invalidRoot}`);
        const postStatePath = this.persistInvalidSszObject("state", postState.serialize(), `${postState.slot}_invalid_state_root_postState_${invalidRoot}`);
