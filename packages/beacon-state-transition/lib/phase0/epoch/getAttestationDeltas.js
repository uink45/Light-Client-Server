...
const util_2 = require("../../allForks/util");
...
function getAttestationDeltas(state, epochProcess) {
    ...
    for (let i = 0; i < statuses.length; i++) {
        ...
        if ((0, util_2.hasMarkers)(status.flags, FLAG_PREV_SOURCE_ATTESTER_OR_UNSLASHED)) {
            rewards[status.proposerIndex] += proposerReward;
            rewards[i] += Math.floor(maxAttesterReward / status.inclusionDelay);
        }
        if ((0, util_2.hasMarkers)(status.flags, FLAG_ELIGIBLE_ATTESTER)) {
            // expected FFG source
            if ((0, util_2.hasMarkers)(status.flags, FLAG_PREV_SOURCE_ATTESTER_OR_UNSLASHED)) {
            ...
            // expected FFG target
            if ((0, util_2.hasMarkers)(status.flags, FLAG_PREV_TARGET_ATTESTER_OR_UNSLASHED)) {
            ...
            // expected head
            if ((0, util_2.hasMarkers)(status.flags, FLAG_PREV_HEAD_ATTESTER_OR_UNSLASHED)) {
