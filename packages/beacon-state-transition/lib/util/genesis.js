...
function getGenesisBeaconState(config, genesisEth1Data, latestBlockHeader) {
    ...
    return (0, allForks_1.createCachedBeaconState)(config, state);
}
 ...
 * @param deposits
 */
function initializeBeaconStateFromEth1(config, eth1BlockHash, eth1Timestamp, deposits, fullDepositDataRootList, executionPayloadHeader = lodestar_types_1.ssz.merge.ExecutionPayloadHeader.defaultTreeBacked()) {
    ...
    if (lodestar_params_1.GENESIS_SLOT >= config.ALTAIR_FORK_EPOCH) {
        const syncCommittees = (0, syncCommittee_1.getNextSyncCommittee)(state, activeValidatorIndices, state.effectiveBalances);
    ...
    if (lodestar_params_1.GENESIS_SLOT >= config.MERGE_FORK_EPOCH) {
        const stateMerge = state;
