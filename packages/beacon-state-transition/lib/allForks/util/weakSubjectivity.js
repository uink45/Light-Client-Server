...
function computeWeakSubjectivityPeriodCachedState(config, state) {
    ...
    return computeWeakSubjectivityPeriodFromConstituents(activeValidatorCount, state.totalActiveBalanceByIncrement, (0, __1.getChurnLimit)(config, activeValidatorCount), config.MIN_VALIDATOR_WITHDRAWABILITY_DELAY);
}
...
function computeWeakSubjectivityPeriod(config, state) {
    ...
    for (const index of activeIndices) {
        totalActiveBalanceByIncrement += Math.floor(state.validators[index].effectiveBalance / lodestar_params_1.EFFECTIVE_BALANCE_INCREMENT);
    }
    if (totalActiveBalanceByIncrement <= 1) {
        totalActiveBalanceByIncrement = 1;
    }
    return computeWeakSubjectivityPeriodFromConstituents(activeIndices.length, totalActiveBalanceByIncrement, (0, __1.getChurnLimit)(config, activeIndices.length), config.MIN_VALIDATOR_WITHDRAWABILITY_DELAY);
}
