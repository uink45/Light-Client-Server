...
async function devHandler(args) {
    ...
    if (args.genesisTime === undefined) {
        await (0, util_1.promisify)(rimraf_1.default)(beaconDbDir);
        await (0, util_1.promisify)(rimraf_1.default)(validatorsDbDir);
    }
    (0, util_2.mkdir)(beaconDbDir);
    (0, util_2.mkdir)(validatorsDbDir);
    ...
    // BeaconNode setup
    const libp2p = await (0, lodestar_1.createNodeJsLibp2p)(peerId, options.network);
    const logger = (0, util_2.getCliLogger)(args, beaconPaths, config);
    ...
    if (args.genesisStateFile) {
        const state = config
            ...
            .BeaconState.createTreeBackedFromBytes(await fs_1.default.promises.readFile(args.genesisStateFile));
    ...
    if (args.startValidators) {
        ...
        const dbPath = path_1.default.join(validatorsDbDir, "validators");
