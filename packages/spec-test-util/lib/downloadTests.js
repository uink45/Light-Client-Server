...
exports.defaultSpecTestsRepoUrl = "https://github.com/ethereum/eth2.0-spec-tests";
...
async function downloadGenericSpecTests({ specVersion, specTestsRepoUrl, outputDir, testsToDownload }, log = logEmpty) {
    ...
    const versionFile = path_1.default.join(outputDir, "version.txt");
    const existingVersion = fs_1.default.existsSync(versionFile) && fs_1.default.readFileSync(versionFile, "utf8").trim();
    ...
    await Promise.all(testsToDownload.map(async (test) => {
        ...
        async () => {
            ...
            await (0, util_1.promisify)(stream_1.default.pipeline)(data, tar_1.default.x({ cwd: outputDir }));
            log(`Downloaded  ${url}`);
        }, {
            retries: 3,
            onRetry: (e, attempt) => {
    ...
    fs_1.default.writeFileSync(versionFile, specVersion);
}
