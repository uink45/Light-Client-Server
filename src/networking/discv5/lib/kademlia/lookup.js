...
class Lookup extends events_1.EventEmitter {
    constructor(config, nodeId, closestPeers) {
        super();
        this.config = config;
        this.target = nodeId;
        this.untrustedEnrs = {};
    ...
    nextPeer() {
        ...
        for (const peer of this.closestPeersByDistance()) {
            if (peer.state === types_1.LookupPeerState.NotContacted) {
                if (atCapacity) {
                    return;
                }
                else {
    ...
    createRpcRequest(peer) {
        const distances = util_1.findNodeLog2Distances(this.target, peer, this.config.lookupRequestLimit);
        return message_1.createFindNodeMessage(distances);
    }
}
