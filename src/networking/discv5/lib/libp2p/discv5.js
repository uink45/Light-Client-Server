...
class Discv5Discovery extends events_1.EventEmitter {
    constructor(options) {
        ...
        this.discv5 = service_1.Discv5.create({
            enr: options.libp2p._config.discv5.enr,
            peerId: options.peerId,
            multiaddr: new multiaddr_1.Multiaddr(options.libp2p._config.discv5.bindAddr),
            config: options,
        ...
        options.libp2p._config.discv5.bootstrapEnrs.forEach((bootEnr) => this.discv5.addEnr(bootEnr));
    }
    async start() {
        if (this.started) {
            return;
